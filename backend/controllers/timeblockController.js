...
/**
 * Private Helper: Check for overlapping blocks
 */
...
/**
 * @desc    Get all timeblocks for a specific day
 * @route   GET /api/timeblock/:dayId
 * @access  Private (Employee)
 */
exports.getTimeBlocksByDay = async (req, res) => {
  try {
...
exports.updateTimeBlock = async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    io.to('admin').emit('timeblock:updated', { 
      block: { ...block.toJSON(), userName: req.user.name } 
    });
